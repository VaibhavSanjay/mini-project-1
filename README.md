...
Any trajectory that shows how to get to the goal state is taken to train an ensemble of conditional Generative Adversarial Networks. The imagined trajectories from GANs are used to train the agent. Similar to Hindsight Experience Replay(HER) where re-labelling of transactions was done.
...
The authors employ a model based approach. As from their problem setting, it is a sparse reward environment where reward is {0, 1}, 1 if it achieves the goal and 0 otherwise.
Imagined trajectories from GANs do not necessarily need to be optimal in the sense of moving directly towards the goal, since the second key component of our method involves feeding these proposed trajectories into a planning algorithm that decides which action to take in order to achieve the goal in as few steps as possible. They specifically use WGANs as these are more stable if training data is non-stationary.
Each of N conditional-GAN had a generator and discriminator. Replay buffer was empty initially and later filled with trajectories from GANs. Sample goals are taken randomly from the replay buffer. During training GANs generated end to end trajectories but not during deployment.
...
Q. What is the use of a Noise Vector in a Generator?

### Future Image Similarity: https://arxiv.org/abs/1910.03157
Application: Navigation

#### Idea
Use expert trajectory to learn a future scene predictor thus not requiring to deploy agent into the environment (ie. no self exploration). This idea is built on top of recent use of  CNNs in RL. BC exists as prior work in this field but requires extensive expert data which this paper is trying to fill in. Finally they created an action-conditioned image predictor using CNN.
...
This problem is similar to zero-trail (only expert demonstrations) imitation learning. The CNN model will generate future image (state) and action. CNN is trained using image-action pairs per timestamp from the expert data. CNN had two neural networks - Enc & Dec to learn future images, while another neural network was trained to learn actions - Act. 
...
#### Why it worked
This is again a way to provide more training data but not using experts or exploration but creating your own model that will generate state - action data. From the way of how many neural networks are thrown at this problem, it looks like it will be very difficult to converge. 
It worked better than BC was due to using the power of CNN to extract out features from expect trajectory image at time t and using the noise of CNNs (that was generated due to only using expert trajectory) to create more imagery exploration.

There isn’t much in their paper, except the main idea, which was novel during 2019.

### Dreamerv3: https://arxiv.org/abs/2301.04104
Application: Video Games, Simulation

#### Idea
<img width="591" alt="Screenshot 2024-10-07 at 4 12 53 PM" src="https://github.com/user-attachments/assets/2ef8c523-edf4-4f09-a7b9-222cb7421fd6">
...
Typical reinforcement learning algorithms often follow an “actor-critic” approach, wherein an actor chooses actions by exploring and using probabilities to maximize a return, while the critic estimates the value function and informs the critic on the quality of its actions. Dreamer introduces a third neural network to this approach: The world model network. All three networks are trained simultaneously. We discuss how these three models interact together to learn a policy.
...
Q1. What is the difference between its World Model of Dreamer V3 and CNN created by “Future Image Similarity”? They kind of do the same thing.

Q2. What is the difference b/w off policy, on policy, model based, model free? How does it make a difference in all of the papers we read?
References

## Table of Critiques
### Dreamer V3
- Learning a neural network that learns the next state was an idea from “Future Image Similarity”. People tried multi-actor to learn an action that is stochastic starting from random and multi-critic algorithms to just differentiate an action from actor and expert. 
- But training more critics will just be overfitting and training more actors doesn’t make sense at all except in cases where your actions are totally contrast like acceleration vs braking in a model that is learning to drift. 
- So the idea of learning the next state instead, from the neural network, helped the algorithm to converge better.
- While “Future Image Similarity” just proposed the idea but never tailored it with other approaches (like mixing with expert data, exploration data to create Replay buffer) thus it didn’t become the first diamond finder but DreamerV3 did. 
### Plan GAN
...
- I doubt that Plan GAN will be overfitted towards the goal that is shown most by the human interaction or was achieved first while training the agent. It will also not scale effectively if a new goal is added into the same environment.
- While prior work HER will work in case of new goal as it will use all existing transactions and rename then goal in those to g’ (new goal added)
### Future Image Similarity
- It sounds very similar to Plan GAN and Dreamer V3. But it is supposed to work poorly then the other two due to the reason that there is no exploration involved. Since the core idea was not to propose a way for better convergence or good reward, rather a way to train without exploration. 
...
- It will not scale on a new environment as the CNNs are trained on image dataset from a particular task. This is big drawback if we compare it to the Dreamer V3
### GenAug
